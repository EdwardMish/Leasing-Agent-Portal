...
const InvitedUserList: React.FC<InvitedUserListProps> = ({ occupant }) => {
    ...
    const noContentMessage = `No invited users exist for ${occupant.name}.`;

    React.useEffect(() => {
        LocationsAPI.getOccupantUsers(occupant.id)
            .then((users: LocationsOccupantUser[]) => {
                setOccupantUsers(users.filter((_) => _.hasInvitation));
