...
const tableColumns: TableTypes.TableColumn[] = [
    {
        display: 'Space',
        width: 'calc(100% - 7rem)',
        operator: IDPlansAPI.ElectricColumns.space,
    },
    meterColumn(IDPlansAPI.ElectricColumns.meterNumber),
];

export const Electric: React.FC<{}> = () => {
    const propertyId: number = React.useContext(PanelContext);

    const [recordsLoaded, setRecordsLoaded] = React.useState<boolean>(false);
    const [records, setRecords] = React.useState<IDPlansAPI.ElectricResponse[]>([]);

    React.useEffect(() => {
        if (propertyId > 0) {
            IDPlansAPI.getElectric(propertyId)
                .then((response) => {
                    setRecords(response);
    ...
    return (
        <IDPlansTableWrapper
            ...
            rowKeys={[
                IDPlansAPI.ElectricColumns.space,
                IDPlansAPI.ElectricColumns.meterNumber,
            ]}
        >
            {(dataRecord) => (
